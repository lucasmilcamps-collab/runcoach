...
- **Riegel a besoin d'un EFFORT, pas d'une moyenne** (`best_sustained_effort`). Une séance structurée prise en entier est une moyenne de choses qui n'ont jamais été courues ensemble : 12 min d'échauffement + 2 km à fond + 11 min de retour au calme donnent 5:49/km, une allure jamais tenue — 22 minutes d'erreur sur un semi. On cherche donc dans les splits la **fenêtre de tours contiguë** la plus rapide.
  - **Fenêtres contiguës, pas « le tour le plus rapide »** : un tour isolé peut être une descente ou une dérive GPS ; une portion tenue à travers plusieurs tours est un effort.
  - **Plancher de 1,5 km** (`MIN_EFFORT_M`, le même seuil que la sélection au niveau activité — un seul, pas deux qui divergent) : sans lui la fonction dégénère en « ton meilleur kilomètre » et transforme chaque footing en record.
  - **Il faut un gain net** (`_EFFORT_GAIN_RATIO`, 10 %) : sinon on garde l'activité entière. Une sortie régulière a toujours une portion un peu rapide ; seule une structure délibérée compte. ⚠️ C'est le **seul** nombre de ce module que ce skill ne fixe pas — un seuil descriptif (« la séance était-elle structurée ? »), pas une constante physiologique.
  - **À allure quasi égale, la fenêtre la plus longue gagne** (`_PACE_TIE_RATIO`) : Riegel dérivant avec le rapport des distances, un 3 km et un 2 km tenus à la même allure ne se valent pas. L'auto-lap qui découpe un effort en kilomètres produit exactement ces quasi-égalités.
- **Une source qui n'est pas un effort vaut moins** : Riegel relie un effort MAXIMAL à un autre. Une course d'entraînement prise en entier en est rarement un, donc l'extrapoler est une affirmation plus faible que d'extrapoler un effort délibéré de même distance — alors que l'échelle de confiance, qui ne lit que le rapport des distances et l'ancienneté, les mettait à égalité. Une source non isolée dégrade donc la `confidence` d'un cran (`downgrade_confidence`, le même mécanisme que pour une allure invraisemblable). Jamais plus d'un cran, et l'estimation est conservée : une course à effort régulier dit tout de même quelque chose.
- **Choisir la source sur la confiance, pas sur l'allure** (`confidence_for` + `CONFIDENCE_ORDER`, utilisés par `_pick_source_effort`). Une fois les efforts isolés, un 2 km à fond bat toujours un 10 km récent à l'allure — et c'est le 2 km qui porte la dérive. On classe donc les candidats par la confiance qu'ils produiraient, l'allure ne départageant qu'à confiance égale : une extrapolation courte n'est retenue que faute de mieux.
- Daniels VDOT (`daniels_vdot`) est disponible comme base d'allures ; le recoupement VO2max n'existe pas encore (donnée Garmin non stockée) — c'est la piste moyen terme, une fois la VO2max Garmin persistée. **C'est aussi le vrai correctif au biais restant** : Riegel depuis 2 km reste optimiste sur un semi, et corriger l'exposant serait inventer un seuil que ce skill ne donne pas. En attendant, la confiance `low` et `feasibility_warning` font le travail.
