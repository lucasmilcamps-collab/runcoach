...
- **Temps par zone FC (`hr_zone_seconds`)** — `enrich_run_zone_seconds`, après la mise à jour du profil FC (dont il dépend) :
  - **Même règle de file que les splits** : une réponse aboutie mais inexploitable (pas de ceinture ce jour-là, vieux boîtier, bandes non reventilables) s'écrit `[]` et sort de la file ; un appel qui **échoue** ne s'écrit pas et sera redemandé. Sans ça l'activité repassait à chaque synchro, et comme le tri va du plus récent au plus ancien elle s'installait en tête et consommait le budget. `compute_trimp` exige cinq valeurs de somme non nulle, donc `[]` retombe proprement sur le repli FC moyenne.
...
- **Splits par tour (`splits`)** — `enrich_run_splits`, même discipline de coût, même forme que les zones (course uniquement, champ manquant uniquement, plus récentes d'abord, plafonné par `_SPLITS_ENRICH_MAX_PER_SYNC`, aucun script de backfill). Deux points propres aux splits :
  ...
  - **Une réponse vide s'écrit `[]`, un appel qui échoue ne s'écrit pas.** Le curseur sélectionne sur `splits: None` : une course sans tours exploitables (tapis, auto-lap coupé) laissée à `None` revient dans la file à **chaque** synchro, et comme le tri est du plus récent au plus ancien elle s'installe en tête et consomme le budget à reposer une question déjà répondue. `[]` la fait sortir de la file. À l'inverse un 500 ne dit rien sur l'activité : `_fetch_splits_sync` ne renvoie que les identifiants dont l'appel a **abouti**, donc un échec reste `None` et sera redemandé. Les deux moitiés sont testées (`test_a_lapless_run_is_asked_once_and_then_left_alone`, `test_a_failed_splits_request_is_retried_next_sync`).
